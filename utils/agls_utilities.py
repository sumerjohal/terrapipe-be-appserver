...
def getEtoFromNCEP(lat, lon, filePath, YYYY_str, MM_str, DD_str):
    ...
    for p in list_of_L3_paths:
        list_of_L5_paths+= [p+'/s2_index__L5='+x for x in s2_index__L5_list 
                            if os.path.exists(p+'/s2_index__L5='+x)]
    
    for q in list_of_L5_paths:
        list_of_L8_paths += [q+'/s2_index__L8='+x for x in s2_index__L8_list 
                             if os.path.exists(q+'/s2_index__L8='+x)]

    #print(list_of_L3_paths)
    #print(list_of_L5_paths)
    #print(list_of_L8_paths)
...
def fetchWKT(geoid):
    ...
    fieldWKT = res.json()['WKT']
    return fieldWKT

def extractLatLon(wkt):
    """
    Extract latitude and longitude pairs from the WKT polygon string.
    
    Args:
    wkt (str): The WKT polygon as a string.
    
    Returns:
    list: A list of tuples containing latitude and longitude pairs.
    """
    lat_lon_pairs = []
    try:
        
        # Remove the "POLYGON" prefix and surrounding parentheses
        ...
        lat_lon_pairs = [tuple(map(float, coord.split())) for coord in coordinates]
    except:
        pass
