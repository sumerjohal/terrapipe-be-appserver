...
def getWeatherFromJRC(agstack_geoid):
    ...
    #                    if os.path.exists(filePath + 's2_token_L8=' + x)]
    # print("=======list_of_8_paths=======",len(list_of_8_paths))
    ...
    print("====shape of res_df before filter========",res_df.shape)

    #screen for the L13
    # res_df = res_df[res_df.s2_index__L13.isin(s2_index_L13_list)].reset_index(drop=True)
    res_df = res_df[res_df.s2_index__L18.isin(s2_index_L18_list)].reset_index(drop=True)


    # filtered_L18 = res_df[res_df.s2_index__L18.isin(s2_index_L18_list)]
    # filtered_L13 = res_df[res_df.s2_index__L13.isin(s2_index_L13_list)]

    # # Combine the filtered DataFrames
    # res_df = pd.concat([filtered_L18, filtered_L13]).drop_duplicates().reset_index(drop=True)
    ...
    # for base_path in list_of_L8_paths:
    #     for root, dirs, files in os.walk(base_path):
    #         # Filter only for Parquet files or folders containing Parquet data
    #         for file in files:
    #             if file.endswith(".parquet"):
    #                 parquet_path = os.path.join(root, file)
    #                 # print(f"======parquet_path====== {parquet_path}")
    #                 try:
    #                     # Read Parquet data and filter based on latitude and longitude
    #                     #weatherDataset = ds.dataset(parquet_path, format="parquet", partitioning="hive")
    #                     weather_df = pq.read_table(parquet_path)
    #                     weather_df = weather_df.to_pandas()
    #                     #weather_df = weather_df[(weather_df['latitude'] == lat) & (weather_df['longitude'] == lon)]
    ...
    # # If data is not empty, return TMF values (integers expected)
    # print(w_all.shape)
    # print(w_all.head())
    # w_all = w_all.sample(n=20, random_state=42)
    # if not w_all.empty:
    #     return w_all[['latitude', 'longitude', 'TMF']]
