...
def get_jrc_description():
    metadata_jrc_description = {
        ...
        "ndvi": "Normalized Difference Vegetation Index (NDVI) value"
    }
...
def getWeatherFromJRC(agstack_geoid):
    filePath = "/media/rnaura/39d0b9c2-7989-4209-8972-db13745755e9/mnt/md2/JRC_2020/PARQUET_S2_1/"
    ...
    for parquet_path in list_of_8_paths:
        weatherDataset = ds.dataset(parquet_path, format="parquet", partitioning="hive")
        weather_df = weatherDataset.to_table(
            filter=(
                (ds.field('latitude') == lat) & (ds.field('longitude') == lon)
            )
        ).to_pandas()
        w_all = pd.concat([w_all, weather_df], ignore_index=True)
    
    if not w_all.empty:
        return w_all[['latitude', 'longitude', 'ndvi']]
...
def getJRCWeatherData():
    ...
    # if end_date is not None:
    #     weather_df = getWeatherFromJRC(geoid, start_date,end_date)
    # else:
    #     weather_df = getWeatherFromJRC(geoid, start_date,end_date)
